#!/usr/bin/env python3
"""
ASR Microservice for Kubernetes Pod (Local Testing Version)
...
def main():
    """Main function for pod operation (local test version)"""
    # Use local directories for testing
    INPUT_DIR = Path("./pod_input")
    OUTPUT_DIR = Path("./pod_output")
    MODEL_PATH = "hindi.pt"
    ...
    print(f"\n{'='*60}")
    print(f"ASR Pod Service (Local Test Mode)")
    print(f"{'='*60}")
    print(f"[{datetime.now()}] Input directory: {INPUT_DIR.absolute()}")
    print(f"[{datetime.now()}] Output directory: {OUTPUT_DIR.absolute()}")
    print(f"{'='*60}\n")
    ...
    print(f"\n[{datetime.now()}] ✓ Service is ready!")
    print(f"[{datetime.now()}] Listening for audio files in {INPUT_DIR.absolute()}...\n")
    processed_files = set()
    ...
    try:
            ...
            for audio_file in audio_files:
                if str(audio_file) not in processed_files:
                    ...
                    try:
                        ...
                        print(f"[{datetime.now()}] 📝 Text: {transcriptions[0][:80]}...")
                        print(f"[{datetime.now()}] ⏱️  Time: {inference_time:.4f}s")
                        print()
        ...
        print(f"\n[{datetime.now()}] ⏹️  Pod service shutting down gracefully...")
        sys.exit(0)
    except Exception as e:
